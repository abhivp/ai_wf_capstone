...
def _model_train(df, prefix, country=None, test=False):
    """
    ...
    # update log
    update_train_log(prefix, country, str(dates[0]), str(dates[-1]), {'rmse': eval_rmse}, runtime, MODEL_VERSION,
                     MODEL_VERSION_NOTE, test=test)


def model_train(data_dir, prefix='sl', test=False):
    """
...
def model_predict(prefix, country, year, month, day, all_models=None, test=False):
    """
    ...
    if not all_models:
        all_data, all_models = model_load(country=country, prefix=prefix, training=False)
